...
test("test automations/listAll returns a list of all automations", () => {
  ...
  // assert
  const result = store.getters["automations/listAll"]() as Automations;
...
test("test automations/initialized returns correct value ", () => {
  ...
  var result = store.getters["automations/initialized"]() as boolean;
...
test("test automations/init stores the items ", () => {
  ...
  // assert values
  const items = store.getters["automations/listAll"]() as Automations;
  automations.forEach((automation, idx) => {
    expect(automation).toEqual(items[idx]);
  });

  const result = store.getters["automations/initialized"]() as boolean;
  expect(result).toEqual(true);
});
