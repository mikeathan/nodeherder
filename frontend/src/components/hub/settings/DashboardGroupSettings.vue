...
const firstDeviceGroupArray = computed(() => {
  const firstGroup = Object.values(dashboardGroups.value)[0];
  return firstGroup ? Object.values(firstGroup.deviceGroup ?? {}) : [];
});
                ...
                <div v-if="expandedDevices[device.deviceId]" class="">
                  <div v-if="device.exposes.length > 0" class="flex flex-wrap gap-2 mb-3">
                    <Chip v-for="expose in device.exposes" :key="expose" :label="expose" removable
                      class="bg-primary-100 text-primary-700" @remove="deleteExpose(group, device.deviceId, expose)" />
                  </div>
