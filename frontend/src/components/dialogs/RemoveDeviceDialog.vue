...
function onRemove() {
  emit('remove', { "id": props.friendlyName, "block": forceRemove.value });
  close();
}
...
<template>
  <Dialog v-model:visible="showDialog" modal header="Rename device" :style="{ width: '25rem' }">
    ...
    <div class="flex items-center gap-4 mb-4">
      <label for="forceRemoveId" class="font-semibold w-24">Force remove</label>
      <ToggleSwitch id="forceRemoveId" v-model="forceRemove" />
    </div>
    <div class="flex justify-end gap-2">
      <Button type="button" label="Cancel" severity="secondary" @click="close()"></Button>
      <Button type="button" label="Remove" @click="(e) => onRemove()"></Button>
    </div>
  </Dialog>
</template>
