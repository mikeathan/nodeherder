...
<template>
  <div class="slider-container" :class="props.direction">
    <BaseSlider
      ...
      <!-- track updates from BaseSlider -->
      <template #extra="{ trackPercentRaw, value: rawValue }">
        <div v-show="false">
          {{ (currentTrackPercent = trackPercentRaw) }}
    ...
    <div
      ...
      :style="
        props.direction === 'vertical' ? { bottom: `${currentTrackPercent}%` } : { left: `${currentTrackPercent}%` }
      ">
      {{ currentValue }} {{ props.unit }}
    </div>
  </div>
</template>
