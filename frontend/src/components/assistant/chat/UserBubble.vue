<script setup lang="ts">
  ...
  const hasChanges = computed(() => editedContent.value.trim() !== props.message.content && editedContent.value.trim().length > 0);
    ...
    <template #content>
      ...
      <div v-else class="w-full flex flex-column gap-2 mt-2">
        ...
        <div class="flex justify-content-end gap-2">
          ...
          <Button label="Update" size="small" class="p-button-sm p-button-rounded" :disabled="!hasChanges" @click="updateAndRun" />
        </div>
      </div>
    </template>
