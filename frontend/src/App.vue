...
function enablePermitJoin() {
  store.dispatch('devices/enablePermitJoin', {
    time: permitJoinDuration,
  });
}

function disablePermitJoin() {
  store.dispatch('devices/disablePermitJoin', {
  });
}
...
const menuItems = [
  ...
  {
    custom: true,
    template: () => h(TimerButton, { duration: permitJoinDuration, startEvent: enablePermitJoin, stopEvent: disablePermitJoin }),
  },
  {
    isLogo: true,
    template: () => h(Logo),
