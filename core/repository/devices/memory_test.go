...
func TestRepositoryCanAddOneDevice(t *testing.T) {
	...
	id := "device 1"
	device, _ := devices.CreateNewDevice(id, "mqtt", createMockPayload(id, 50, 60.1, 23.5, 120.0))
...
func TestRepositoryCanAddMultipleDevices(t *testing.T) {
	...
	repo := repository.NewMemoryDeviceRepo()
	devId1 := "device 1"
	device1, _ := devices.CreateNewDevice(devId1, "mqtt", createMockPayload(devId1, 50, 60.1, 23.5, 120.0))

	devId2 := "device 2"
	device2, _ := devices.CreateNewDevice(devId2, "mqtt", createMockPayload(devId2, 90, 34.7, 36.2, 56.0))
...
func TestRepositoryCanUpdateExistingDevice(t *testing.T) {
	...
	device1b, _ := devices.CreateNewDevice(devId1, "mqtt", createMockPayload(devId1, 90, 34.7, 36.2, 56.0))
	repo.Store(devId1, device1)
